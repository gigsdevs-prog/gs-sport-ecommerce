...
export default function Page() {
  ...
  const quickLinks = [
    { href: '/account/orders', label: t('my_orders'), icon: Package, desc: t('track_manage_orders') },
    { href: '/wishlist', label: t('wishlist'), icon: Heart, desc: t('saved_products') },
    { href: '/account', label: t('edit_profile'), icon: User, desc: t('update_your_info') },
  ];
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                {editing ? (
                  <div className="flex items-center gap-2">
                    <input value={fullName} onChange={e => setFullName(e.target.value)} className="bg-white/10 border border-white/20 rounded px-3 py-1.5 text-white text-lg outline-none focus:border-white/40" placeholder={t('your_name')} />
                    <button onClick={handleSaveProfile} className="text-xs bg-white text-black px-3 py-1.5 rounded font-medium">{t('save')}</button>
                    <button onClick={() => setEditing(false)} className="text-xs text-white/60 hover:text-white">{t('cancel')}</button>
                  </div>
                ) : (
                  <>
                    <h1 className="text-xl md:text-2xl font-light tracking-wide">{profile?.full_name || t('welcome')}</h1>
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-white/60 flex-wrap">
                ...
                {isAdmin && <span className="flex items-center gap-1.5 text-yellow-400"><Shield size={14} />{t('admin')}</span>}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-neutral-100 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-neutral-100">
              <h2 className="text-sm font-medium tracking-wide">{t('recent_orders')}</h2>
            ...
            {recentOrders.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-neutral-400 mb-4">{t('no_orders')}</p>
                <Link href="/shop"><Button size="sm">{t('start_shopping')}</Button></Link>
              </div>
            ) : (
          ...
          {/* Quick Links / Admin Panel */}
          <div className="space-y-4">
            {quickLinks.map(link => (
              <Link key={link.href} href={link.href} className="flex items-center gap-3 p-4 bg-white border border-neutral-100 rounded-xl hover:shadow-md transition-all group">
                <div className="w-10 h-10 rounded-lg bg-neutral-50 flex items-center justify-center group-hover:bg-neutral-100 transition-colors">
                  <link.icon size={18} className="text-neutral-400" />
                </div>
                <div>
              ...
              <Link href="/admin" className="flex items-center justify-between p-4 bg-neutral-900 text-white rounded-xl hover:bg-neutral-800 transition-all group">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-white/50 mt-0.5">{t('manage_your_store')}</p>
                  </div>
                </div>
